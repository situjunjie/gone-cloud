...
import cn.iocoder.yudao.module.devops.service.kubernetes.terminal.KubernetesTerminalService;
import cn.iocoder.yudao.module.devops.service.kubernetes.terminal.KubernetesTerminalSession;
...
/**
 * Kubernetes Pod 终端 WebSocket 处理器。
 */
@Slf4j
@Component
public class KubernetesTerminalWebSocketHandler extends TextWebSocketHandler {

    private static final String ATTRIBUTE_TERMINAL_CONNECTION = "KUBERNETES_TERMINAL_CONNECTION";

    @Resource
    private KubernetesTerminalService kubernetesTerminalService;
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        try {
            ...
            KubernetesTerminalSession terminalSession = openTerminalWithTenant(session, params);
            ...
            log.error("[afterConnectionEstablished][uri({}) Kubernetes terminal open failed]", session.getUri(), ex);
            sendMessage(session, TYPE_ERROR, null, null, "Kubernetes 终端连接失败");
    ...
    public void handleTransportError(WebSocketSession session, Throwable exception) throws Exception {
        log.warn("[handleTransportError][session({}) Kubernetes terminal transport error]", session.getId(), exception);
    ...
    private void startOutputForwarding(TerminalConnection connection) {
        terminalExecutor.execute(() -> forwardOutput(connection, connection.getTerminalSession().getExecWatch().getOutput()));
        terminalExecutor.execute(() -> forwardOutput(connection, connection.getTerminalSession().getExecWatch().getError()));
    }

    private KubernetesTerminalSession openTerminalWithTenant(WebSocketSession session, MultiValueMap<String, String> params)
            throws Exception {
        KubernetesTerminalSession[] terminalSession = new KubernetesTerminalSession[1];
        executeWithTenant(session, () -> terminalSession[0] = kubernetesTerminalService.openTerminal(
                parseLong(params.getFirst("environmentId")), params.getFirst("namespace"),
                params.getFirst("podName"), params.getFirst("containerName")));
        return terminalSession[0];
    }

    private void forwardOutput(TerminalConnection connection, InputStream stream) {
        if (stream == null) {
            return;
        }
        ...
        try {
            ...
            if (!connection.getClosed().get()) {
                log.warn("[forwardOutput][session({}) Kubernetes terminal output failed]",
    ...
    private void closeTerminal(WebSocketSession session, String reason, boolean closeWebSocket) {
        ...
        try {
            ...
            log.warn("[closeTerminal][session({}) close Kubernetes terminal failed]", session.getId(), ex);
        }
    ...
    private static class TerminalConnection {
        ...
        private final KubernetesTerminalSession terminalSession;
